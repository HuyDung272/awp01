...
export class UserService {
  ...
  private getroleUrl = '/api/roles';
  private getPMUserUrl = '/api/pmusers';
  ...
  getRoles(): Observable<any> {
    return this.http.get(this.getroleUrl).map(res => res.json());
  }

  getPMUsers(user: any): Observable<any> {
    return this.http.get(this.getPMUserUrl + `/${user._id}`).map(res => res.json());
  }
